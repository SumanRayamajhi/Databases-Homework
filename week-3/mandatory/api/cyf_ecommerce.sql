...
--If you don't have it already, add a new GET endpoint `/products` to load all
-- the product names along with their supplier names.
...
--name using a query parameter, for example `/products?name=Cup`. 
...
--be returned: order references, order dates, product names, unit prices, suppliers and quantities.
select o.order_reference, o.order_date, p.product_name, oi.quantity, customer_id from orders o
